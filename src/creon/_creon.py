...
class Creon:
    def __init__(self):
        ...
        self.obj_Dscbo1_StockIndexIR = win32com.client.Dispatch("Dscbo1.StockIndexIR")

        # contexts
        self.stockcur_handlers = {}  # 주식/업종/ELW시세 subscribe event handlers
        self.orderevent_handler = None

    def connect(self, id_, pwd, pwdcert, c_path, trycnt=300):
        if not self.connected():
            app = application.Application()
            app.start(c_path + f" /id:{id_} /pwd:{pwd} /pwdcert:{pwdcert} /autostart")
    ...
    def get_stockfeatures(self, code):
        ...
        me_const = constants.MarketEye
        _fields = list(me_const.values())
        _keys = list(me_const.keys())
        # print(len(me_const))

        # fmt: off
        # _fields = [20, 21, 67, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85,
        #           86, 87, 88, 89, 90, 91, 92, 93, 94, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106,
        #           107, 108, 109, 110, 116, 118, 120, 123, 124, 125, 127, 156]
        # _keys = ['총상장주식수', '외국인보유비율', 'PER', '시간외매수잔량', '시간외매도잔량', 'EPS', '자본금', '액면가',
        #           '배당률', '배당수익률', '부채비율', '유보율', '자기자본이익률', '매출액증가율', '경상이익증가율', '순이익증가율',
        #           '투자심리', 'VR', '5일회전율', '4일종가합', '9일종가합', '매출액', '경상이익', '당기순이익', 'BPS',
        #           '영업이익증가율', '영업이익', '매출액영업이익률', '매출액경상이익률', '이자보상비율', '분기BPS',
        #           '분기매출액증가율', '분기영업이액증가율', '분기경상이익증가율', '분기순이익증가율', '분기매출액', '분기영업이익',
        #           '분기경상이익', '분기당기순이익', '분개매출액영업이익률', '분기매출액경상이익률', '분기ROE', '분기이자보상비율',
        #           '분기유보율', '분기부채비율', '프로그램순매수', '당일외국인순매수', '당일기관순매수', 'SPS', 'CFPS', 'EBITDA',
        #           '공매도수량', '당일개인순매수']
        # fmt: on
